        ...
        return function(options) {
            ...
            return class extends HTMLElement {
                ...
                constructor() {
                    ...
                    this.event.created && this.event.created();
                }

                attributeChangedCallback(name, prev, next) {
                    this.setup.updated && this.setup.updated(name, prev, next, "attrs");
                    this.event.updated && this.event.updated(name, prev, next, "attrs");
                }

                connectedCallback() {
                    this.setup.mounted && this.setup.mounted();
                    this.event.mounted && this.event.mounted();
                }
                        ...
                        Object.defineProperty(this.props, property, {
                            set: (next) => {
                                ...
                                if (prev !== next) {
                                    ...
                                    this.render();
                                    this.setup.updated && this.setup.updated(property, prev, next, "props");
                                    this.event.updated && this.event.updated(property, prev, next, "props");
                                }
                            },
                            get: () => {
                        ...
                        Object.defineProperty(this.state, current, {
                            set: (next) => {
                                ...
                                if (prev !== next) {
                                    ...
                                    this.render();
                                    this.setup.updated && this.setup.updated(current, prev, next, "state");
                                    this.event.updated && this.event.updated(current, prev, next, "state");
                                }
                            },
                            get: () => {
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                if (type === "props")
                    switch (name) {
                        case "closed":
                            this.props[name] = this.truty(next);
                            break;
                    }

                if (type === "props") {
                    this.setter(name, next, ["closed"]);
                    switch (name) {
                        case "closed":
    ...
    return OS.$Component({
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                ...
                if (type === "props") {
                    this.setter(name, next, ["horisontal", "vertical"]);
                    switch (name) {
                        case "vertical":
                        case "horisontal":
                            this.emit("change:" + name, { data: next });
    ...
    return OS.$Component({
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                ...
                if (type === "props") {
                    this.setter(name, next, ["theme", "timer"]);
                    switch (name) {
                        case "theme":
                        case "timer":
                            this.emit("change:" + name, { data: next });
    ...
    return OS.$Component({
        ...
        setup: {
            updated(name, prev, next, type) {
                ...
                if (type === "props") {
                    this.setter(name, next, ["fixed", "align", "transparent", ]);
                    switch (name) {
                        case "fixed":
                        case "align":
                        case "transparent":
    ...
    return OS.$Component({
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                if (type === "attrs")
                    switch (name) {
                        case "label":
                            ...
                            break;
                        case "expand":
                            this.props.expand = this.truty(next);
                            break;
                    }
                ...
                if (type === "props") {
                    this.setter(name, next, ["label", "expand"]);
                    switch (name) {
                        case "label":
                            this.emit("change:" + name, { data: next });
                            break;
    ...
    return OS.$Component({
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                ...
                if (type === "props") {
                    this.setter(name, next, ["label", "position"]);
                    switch (name) {
                        case "label":
                        case "position":
                            this.emit("change:" + name, { data: next });
                        ...
                        case "expand":
                            next
                                ?
                                (this.state.show = next, this.rules.pos()) :
                                setTimeout(() => {
                                    this.state.show = next;
                                }, 250);
                            break;
                ...
                if (type === "attrs")
                    switch (name) {
                        case "position":
                            ...
                            break;
                        case "fixed":
                            this.props.fixed = this.truty(next);
                            break;
                    }
                ...
                if (type === "state")
                    switch (name) {
                        case "show":
                            ...
                            this.emit("change:expand", { data: next, });
                            break;
                    }

                if (type === "props") {
                    this.setter(name, next, ["position", "fixed"]);
                    switch (name) {
                        case "fixed":
            ...
            <html ref="page" part="page" lang="{{ @props.locale }}" dir="{{ @props.locale === 'ar' ? 'rtl' : 'ltr' }}">
                ...
                <body>
                    <style>
                        ...
                        @page { size: {{ @props.size }}; margin: {{ @props.margin }}; }
                        ...
                        .table-body-col { font-size: {{ @theme.fonts.sizes("BASE") }}; line-height: {{ @theme.fonts.lines("BASE") }}; }
                        .table-head-col, .table-body-col, #table-empty-col { padding: .5rem 1rem; color: {{ @theme.colors("OS-BLACK") }}; }
                        .table-head-col { font-weight: 700; font-size: {{ @theme.fonts.sizes("SMALL") }}; line-height: {{ @theme.fonts.lines("SMALL") }}; }
                        .table-body-row, #table-empty-row { border-top-width: 1px; border-top-style: solid; border-top-color: {{ @theme.colors("OS-SHADE") }}; }
                        #title { font-weight: 700; text-align: center; margin: 0 0 1rem 0; font-size: {{ @theme.fonts.sizes("XLARGE") }}; line-height: {{ @theme.fonts.lines("XLARGE") }}; }
                        #table-empty-col { font-weight: 700; padding: 1rem 2rem; text-align: center; font-size: {{ @theme.fonts.sizes("BASE") }}; line-height: {{ @theme.fonts.lines("BASE") }}; }
                        #wrapper { width: 100%; border-width: 1px; border-style: solid; border-radius: .25rem;  background: {{ @theme.colors("OS-WHITE") }}; border-color: {{ @theme.colors("OS-SHADE") }}; }
                    </style>
                    <slot ref="printStyles" name="styles" />
                    <table id="page">
                        <thead>
                            <tr>
                                <td>
                                    <div id="header">
                                        <slot ref="printHeader" name="header" />
                                    </div>
                                </td>
                            </tr>
                        </thead>
                            ...
                            <tr>
                                <td>
                                    <main id="main">
                                        ...
                                        <slot ref="printBottom" name="bottom" />
                                    </main>
                                </td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr>
                                <td>
                                    <div id="footer">
                                        <slot ref="printFooter" name="footer" />
                                    </div>
                                </td>
                            </tr>
                        </tfoot>
    ...
    return OS.$Component({
        ...
        rules: {
            print() {
                ...
                const page = (new DOMParser).parseFromString(this.refs.page.outerHTML, "text/html");
                page.head.appendChild(page.querySelector("style"));
                [
                    { id: "top", els: this.refs.printTop.assignedNodes() },
                    { id: "styles", els: this.refs.printStyles.assignedNodes() },
                    { id: "header", els: this.refs.printHeader.assignedNodes() },
                    { id: "bottom", els: this.refs.printBottom.assignedNodes() },
                    { id: "footer", els: this.refs.printFooter.assignedNodes() },
                ].map(e => {
                    e.els.forEach(el => {
                        const _el = el instanceof HTMLElement ?
                            el.cloneNode(true) : el.textContent;
                        if (e.id === "styles") page.head.appendChild(_el);
                        else page.querySelector(`[name=${e.id}]`)[typeof _el === "string" ? "insertAdjacentHTML" : "insertAdjacentElement"]("beforebegin", _el);
                    });
                    page.querySelector(`[name=${e.id}]`).remove();
                });
                this.state.iframe.open();
                this.state.iframe.write(page.documentElement.outerHTML);
                ...
                if (type === "attrs")
                    switch (name) {
                        ...
                        case "placeholder":
                            ...
                            break;
                        case "disabled":
                            this.props.disabled = this.truty(next);
                            break;
                    }

                if (type === "props") {
                    this.setter(name, next, ["label", "target", "disabled", "placeholder", ]);
                    switch (name) {
                        case "label":
                        case "target":
                        case "disabled":
...
OS.$Component.Printable = (function() {
    const Style = /*css*/ `
        :host {
            display: none;
        }
    ...
    const Template = /*html*/ `
        <html ref="page" part="page" lang="{{ @props.locale }}" dir="{{ @props.locale === 'ar' ? 'rtl' : 'ltr' }}">
            ...
            <body>
                ...
                <slot ref="printStyles" name="styles" part="styles" />
                <table id="page">
                    <thead>
                        <tr>
                            <td>
                                <div id="header">
                                    <slot ref="printHeader" name="header" part="header" />
                                </div>
                            </td>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>
                                <main id="main">
                                    <slot ref="printTop" name="top" part="top" />
                                    <slot ref="printMain" part="main" />
                                    <slot ref="printBottom" name="bottom" part="bottom" />
                                </main>
                            </td>
                        </tr>
                    </tbody>
                    <tfoot>
                        <tr>
                            <td>
                                <div id="footer">
                                    <slot ref="printFooter" name="footer" part="footer" />
                                </div>
                            </td>
                        </tr>
                    </tfoot>
    ...
    return OS.$Component({
        tag: OS.$Selectors.Printable,
        tpl: Template,
        css: [Style],
    })({
        attrs: ["margin", "size", "locale"],
        props: {
            margin: "5mm 5mm 5mm 5mm",
            size: "A4",
            locale: document.documentElement.lang,
        ...
        rules: {
            print() {
                ...
                [
                    { id: "top", els: this.refs.printTop.assignedNodes() },
                    { id: "main", els: this.refs.printMain.assignedNodes() },
                    { id: "styles", els: this.refs.printStyles.assignedNodes() },
                    { id: "header", els: this.refs.printHeader.assignedNodes() },
                    { id: "bottom", els: this.refs.printBottom.assignedNodes() },
                    { id: "footer", els: this.refs.printFooter.assignedNodes() },
                    ...
                    e.els.forEach(el => {
                        const _el = el instanceof HTMLElement ?
                            el.cloneNode(true) : el.textContent;
                        if (e.id === "styles") page.head.appendChild(_el);
                        else page.querySelector(`[part=${e.id}]`)[typeof _el === "string" ? "insertAdjacentHTML" : "insertAdjacentElement"]("beforebegin", _el);
                    });
                    page.querySelector(`[part=${e.id}]`).remove();
                });
    ...
    return OS.$Component({
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                ...
                if (type === "props") {
                    this.setter(name, next, ["value", "group", "checked", "disabled"]);
                    switch (name) {
                        case "value":
                        case "group":
                        case "disabled":
                            ...
                            break;
                        case "checked":
                            if (next && this.props.group.length) {
    ...
    return OS.$Component({
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                ...
                if (type === "props") {
                    this.setter(name, next, ["type", "label", "disabled", "placeholder"]);
                    switch (name) {
                        case "type":
                        case "label":
                        case "disabled":
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                ...
                if (type === "state")
                    switch (name) {
                        case "type":
                            if (!["text", "password"].includes(next)) {
                                this.state.type = "password";
                            } else {
                ...
                if (type === "props") {
                    this.setter(name, next, ["label", "disabled", "placeholder"]);
                    switch (name) {
                        case "label":
                        case "disabled":
                        case "placeholder":
    ...
    return OS.$Component({
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                ...
                if (type === "props") {
                    this.setter(name, next, ["rows", "label", "disabled", "placeholder"]);
                    switch (name) {
                        case "rows":
                        case "label":
                        case "disabled":
                ...
                if (type === "props") {
                    ...
                    switch (name) {
                        ...
                        case "setValue":
                            ...
                            break;
                        case "query":
                            if (this.falsy(next, [""])) this.props.value = "";
                            this.refs.field.value = next;
                            this.emit("change:" + name, { data: next });
                            break;
                        case "value":
    ...
    return OS.$Component({
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                if (type === "attrs")
                    switch (name) {
                        ...
                        case "disabled-minutes":
                            this.props.disabledMinutes = (next || "")
                                .split(",")
                                .map((e) => e.trim())
                                .filter((e) => e.length);
                            break;
                ...
                if (type === "props") {
                    this.setter(name, next, ["label", "format", "disabled", "placeholder"]);
                    switch (name) {
                        case "label":
                        case "format":
                        case "placeholder":
                        ...
                        case "disabled":
                            ...
                            this.emit("change:disabled", { data: next });
                            break;
                    }
                }
                ...
                <div ref="content" part="content" @click:propagation="{{ () => {} }}">
                    ...
                    ($ if !["Y", "M"].includes(@state.view) $)
                        <ul ref="control" part="control">
                            <button ref="trigger_prev" part="trigger" type="button" @click="{{ @rules.prev }}" @keydown:propagation="{{ () => {} }}">
                            ...
                            <button ref="trigger_next" part="trigger" type="button" @click="{{ @rules.next }}" @keydown:propagation="{{ () => {} }}">
                    ...
                    <div ref="items" part="items">
                        ($ if ["Y", "M"].includes(@state.view) $)
                            ...
                            ($ range value into range[0] to range[1] $)
                                <button ref="date" part="date" type="button" 
                                    data-next="D" 
                                    data-value="{{ value }}" 
                        ...
                        ($ else $)
                            ($ each day into @state.info.array $)
                                ...
                                <button ref="date" part="date" type="button" @click="{{ @rules.change }}" @keydown:propagation="{{ () => {} }}" data-date="{{ date }}" {{> attr ? attr + '="true"' : "" }}>
                                    {{ text }}
                                </button>
                            ($ endeach $)
                        ($ endif $)
                ...
                if (type === "attrs")
                    switch (name) {
                        ...
                        case "disabled":
                            ...
                            break;
                        case "full-day":
                            this.props.fullDay = next || true;
                            break;
                        case "full-month":
                            this.props.fullMonth = next || true;
                            break;
                        case "year-range":
                            var range = (next || "")
                                .split(",")
    ...
    return OS.$Component({
            ...
            remove(event) {
                ...
                this.emit("delete", { data: file }, () => {
                    if (type === "files") {
                        ...
                        this.props.value = "";
                        if (this.state.data.files.length)
                            this.props.value = this.state.data.files;
                    } else {
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                ...
                if (type === "props") {
                    this.setter(name, next, ["accept", "source", "multiple", "disabled"]);
                    switch (name) {
                        case "accept":
                        case "source":
                        case "disabled":
                        ...
                        case "multiple":
                            ...
                            break;
                        case "value":
                            if (this.falsy(next, [""])) this.ctl.setFormValue(null);
                            else {
                                var value = next;
    ...
    return OS.$Component({
        ...
        rules: {
            sub() {
                ...
                this.emit("sub", { data: this.props.value });
            },
            add() {
                if ((this.props.max && this.props.value < this.props.max) || !this.props.max)
                    this.props.value = +(this.props.value + this.props.step);
                this.emit("add", { data: this.props.value });
            },
            focus() {
                this.emit("focus", { data: this.props.value });
            },
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                ...
                if (type === "props") {
                    this.setter(name, next, ["label", "min", "max", "step", "disabled", "placeholder"]);
                    switch (name) {
                        case "max":
                        case "step":
                        case "label":
    ...
    return OS.$Component({
        ...
        setup: {
            ...
            updated(name, prev, next, type) {
                ...
                if (type === "props") {
                    this.setter(name, next, ["type", "vertical", "outlined", "disabled", ]);
                    switch (name) {
                        case "type":
                        case "vertical":
                        case "outlined":
