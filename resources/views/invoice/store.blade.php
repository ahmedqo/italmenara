...
@section('content')
    <div class="flex flex-col gap-2">
        ...
        <div class="bg-x-white rounded-x-huge shadow-x-core p-6">
            <form action="{{ route('actions.invoices.store') }}" method="POST" class="w-full flex flex-col gap-6">
                @csrf
                <div class="w-full grid grid-rows-1 grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <os-select label="{{ __('Type') }}" name="type">
                        @foreach (Core::invoiceTypeList() as $type)
                            <os-option value="{{ $type }}">{{ __(ucwords($type)) }}</os-option>
                        @endforeach
                    </os-select>
                    <os-text label="{{ __('Reference') }}" name="reference"
                        value="{{ $data ? $data->reference : old('reference') }}"></os-text>
                    <os-text type="number" label="{{ __('Charges') }}" name="charges"
                        value="{{ $data ? $data->charges : old('charges') }}"></os-text>
                </div>
                ...
                <div class="w-full grid grid-rows-1 grid-cols-1 lg:grid-cols-2 gap-4 items-start">
                    <os-area rows="2" label="{{ __('Note') }} (en)" name="note_en"
                        value="{{ $data ? $data->note_en : old('note_en') }}"></os-area>
                    <os-area rows="2" label="{{ __('Note') }} (it)" name="note_it"
                        value="{{ $data ? $data->note_it : old('note_it') }}"></os-area>
                    <os-area rows="2" label="{{ __('Note') }} (fr)" name="note_fr"
                        value="{{ $data ? $data->note_fr : old('note_fr') }}"></os-area>
                    <os-area rows="2" label="{{ __('Note') }} (ar)" name="note_ar"
                        value="{{ $data ? $data->note_ar : old('note_ar') }}"></os-area>
                </div>
