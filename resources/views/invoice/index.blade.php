...
@section('content')
    <div class="flex flex-col gap-2">
        <os-data-visualizer print search filter download title="{{ __('Invoices List') }}">
            <a slot="end" title="{{ __('Create') }}" href="{{ route('views.invoices.store') }}"
...
@section('scripts')
    <script>
        ...
        TableVisualizer(dataVisualizer, 'invoices', {
            Currency: "{{ Core::getSetting('currency') }}",
            Search: "{{ route('actions.invoices.search') }}",
            Scene: "{{ route('views.invoices.scene', 'XXX') }}",
            Print: "{{ route('views.invoices.scene', ['id' => 'XXX', 'print' => 'true']) }}",
            Patch: "{{ route('views.invoices.patch', 'XXX') }}",
            Clear: "{{ route('actions.invoices.clear', 'XXX') }}",
            Csrf: "{{ csrf_token() }}",
        });
    </script>
@endsection
