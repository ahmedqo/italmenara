...
@section('content')
    <div class="flex flex-col gap-2">
        <os-data-visualizer print search filter download title="{{ __('Products List') }}">
            <a slot="end" title="{{ __('Create') }}" href="{{ route('views.products.store') }}"
...
@section('scripts')
    <script>
        ...
        TableVisualizer(dataVisualizer, 'products', {
            Currency: "{{ Core::getSetting('currency') }}",
            Search: "{{ route('actions.products.search') }}",
            Patch: "{{ route('views.products.patch', 'XXX') }}",
            Clear: "{{ route('actions.products.clear', 'XXX') }}",
            Csrf: "{{ csrf_token() }}",
        });
    </script>
@endsection
