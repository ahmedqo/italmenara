...
@section('content')
    <div class="flex flex-col gap-2">
        <os-data-visualizer print search filter download title="{{ __('Quotations List') }}">
            <a slot="end" title="{{ __('Create') }}" href="{{ route('views.quotations.store') }}"
...
@section('scripts')
    <script>
        ...
        TableVisualizer(dataVisualizer, 'quotations', {
            Currency: "{{ Core::getSetting('currency') }}",
            Search: "{{ route('actions.quotations.search') }}",
            Scene: "{{ route('views.quotations.scene', 'XXX') }}",
            Print: "{{ route('views.quotations.scene', ['id' => 'XXX', 'print' => 'true']) }}",
            Patch: "{{ route('views.quotations.patch', 'XXX') }}",
            Clear: "{{ route('actions.quotations.clear', 'XXX') }}",
            Csrf: "{{ csrf_token() }}",
        });
    </script>
@endsection
